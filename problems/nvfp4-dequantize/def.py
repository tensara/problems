...
class nvfp4_dequantize(Problem):

    is_exact = False

    parameters = [
        {"name": "q", "type": "uint8_t", "pointer": True, "const": True},
        {"name": "scale", "type": "float8", "pointer": True, "const": True},
    ...
    def __init__(self):
        super().__init__(name="nvfp4-dequantize")

    def reference_solution(self, q: torch.Tensor, scale: torch.Tensor, sf_g: float) -> torch.Tensor:
    ...
    def _make_case(self, m: int, k: int, name: str) -> Dict[str, Any]:
        ...
        def create_inputs(m=m, k=k, seed=seed):
            ...
            sf_g_t = torch.tensor([sf_g], device=a.device, dtype=torch.float32)
            q, scale = _nvfp4_quantize(a, sf_g_t)
            return q, scale, sf_g
